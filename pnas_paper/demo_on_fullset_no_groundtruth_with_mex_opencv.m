function demo
%This function demonstrate all steps for processing all the cores. However,
%all the steps are using mex_opencv rather than MATLAB for fasting
%computing
    clc;
    clear all;
    close all;
    ...
    for fileIdx = 1:nfiles
        ...
        %if (~exist('fimbifilter_1024'))
              idx = find(fim_1024~=(-3));
              sigmar = 10;
              sigmad = 0.5*std(fim_1024(idx));
              fim_1024=cast(fim_1024,'single');
              %Run bilateral filtering on opencv_mex
              fimbifilter_1024 = bilateralFilter(fim_1024,'Diameter',round(3*sigmar),'SigmaColor',sigmad,'SigmaSpace',sigmar);
