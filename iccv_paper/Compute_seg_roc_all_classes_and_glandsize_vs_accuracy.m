               ...
               for sampleIdx=1:nsamples
                    ...
                    if (exist(seg_im_name_gt,'file')&exist(seg_im_name,'file'))
                        ...
                        [sorted_area,I] = sort(gland_area_arr,'descend');
                           ...
                           figure(2);
                           plot(area_coord,hist_val,'-b','linewidth',2);hold on;
                           nglandsperinterval = ceil(length(sorted_accu)/nbins);
                           mean_accu_hist =imresize(sorted_accu,[1 nbins]);
                           figure(2);
                           %errorbar(area_coord,mean_accu_hist,std_accu_hist,'-r','linewidth',2);hold off;
                           plot(area_coord,mean_accu_hist,'-r','linewidth',2);hold off;
...
save('area_vs_accuracy.mat','area_coord','mean_accu_hist','hist_val','sorted_area','sorted_accu');
