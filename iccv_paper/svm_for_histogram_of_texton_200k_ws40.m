...
svm_dir = 'H:\QPI data\svm\texton_ws40_1024\';
addpath(svm_dir);
addpath('C:\Users\thnguyn2\Dropbox\current working code\Bilateral filtering\');%Folder for bilateral filtering
...
if (~exist('svmstruct_ws40.mat','file'))
    if (~exist('sampleddata_ws40.mat','file'))
        ...
        case 2
            for classidx=1:4 %Go through different classes
               ...
               for sampleIdx=1:nsamples
                    ...
                    disp(['Adding data of: ' label_name ' ...']);
                    tx_hist_file_name = strcat(label_name,'_texton_hist_40.mat');
                    svm_file_name = strcat(label_name,'_svm_200k_ws40_1024.mat');
                    if (~exist(strcat(svm_dir,svm_file_name),'file'))
                        nrows = 1024;
                        ncols = 1024;
                        load(strcat(data_dir,tx_hist_file_name),'histim');
                        load(strcat(label_dir,label_name,'_resized.mat'));
                        ...
                        tempidx = find(lblim~=0);
                        newlblim_1024 = lblim;
                        newlblim_1024(tempidx)=outputval;
                        groundtruth = lblim(tempidx);
                        fim_1024(tempidx)=f;
                        figure(1);
                        subplot(121);
                        imagesc(reshape(newlblim_1024,[nrows ncols]));
                        title('SVM results');
                        subplot(122);
                        ...
                        colorbar;
                        title('f');
                        save(svm_file_name,'fim_1024','newlblim_1024','f','outputval','lblim');
                     else
                        load(svm_file_name,'fim_1024','lblim');
                        %Perform bilateral filtering
                        idx = find(fim_1024~=(-3));
                        fim_1024bf = bifilter(fim_1024,10,0.5*std(fim_1024(idx)));
                        save(svm_file_name,'fim_1024bf','-append');
                        figure(1);
                        imagesc([fim_1024 fim_1024bf]);
                        drawnow;
                     end
               end
            end
