%This program produces the SVM classification results for histogram of
%directions
clc;
clear all;
close all;
...
%save filesnames.mat filenames;
histpath = 'F:\QPI data\text_dir_hist\';
label_dir='F:\QPI data\label\';
svm_dir = 'H:\QPI data\svm\ws20\';
...
if (~exist('svmstruct_hod_ws40.mat','file'))
    if (~exist('sampleddata_hod_ws40.mat','file'))
        for classidx=1:4 %Go through different classes
               ...
               for sampleIdx=1:nsamples
                    ...
                    gland_hist = zeros(ngland,ndirs);
                    stroma_hist = zeros(nstroma,ndirs);

                    for bandidx=1:ndirs
                        curband = rotinvarhist(:,:,bandidx);
        ...
        save('sampleddata_hod_ws40.mat','data','label','filelist');
    else
        load('sampleddata_hod_ws40.mat');
    end
    ...
    save 'svmstruct_hod_ws40.mat' svmtruct;

else
    %Testing the classifier--To be modified
    disp('Loading trained classifier...')
    load('svmstruct_hod_ws40.mat');
    disp('Loading validation data...');
     switch testmode
         case 1 %Perform validation on the training set
            load('sampleddata_hod_ws40.mat','data','label');
            ...
            disp(['Cross-validation accuracy' num2str(accu_rate)]);
         case 2
            load filesnames.mat; %Needs to be commented out in other program...
            for classidx=1:4 %Go through different classes
               nsamples = (length(filenames{classidx,1})); %Get the number of samples in each class
               for sampleIdx=1:nsamples
                     nfilestotal =nfilestotal+1; 
                    ...
                    load(strcat(label_dir,label_name,'_resized.mat'));
                    load(strcat(histpath,texture_dir_hist_file_name));
                    svm_file_name = strcat(label_name,'_hod_svm_200k_ws40.mat');
                    if (~exist(svm_file_name))
                         ...
                         newhistim = zeros(nrows,ncols,ndirs);
                         ...
                         testdata = zeros(nrows*ncols,ndirs);
                         for bandidx=1:ndirs
                             newhistim(:,:,bandidx)=imresize(rotinvarhist(:,:,bandidx),[nrows,ncols],'nearest');
                         end
                         histim = newhistim;
                         ...
                         clear histim;
                         clear newhistim;
                         %Now, justclassify gland vs stroma
