function diagnosis_feature_analysis
    ...
    [filenames2,glandnames2,gradelist2]=findFileNameFromROIsWithCoreDiagnosis(datapath);
  
    ngrades = size(gradelist2,1);
    samplename2 = cell(0,1);
    %The following section of the code reads all the files
    for classidx=1:ngrades %Go through different classes
       if (~ismember(gradelist2(classidx),{'NC','SK'})) %Only check the cancer case
               nsamples = (length(filenames2{classidx,1})); %Get the number of samples in each class
    ...
    if (strcmp(mode,'gt')==1)
        ...
        %save('glandmorpfeat_no_label.mat','glandmorpfeat','classvect','samplename','indivglandfeat');
        load('glandmorpfeat_no_label.mat');
    end
        
    if (usecorediagnoisresult)
        [C,I1,I2]=intersect(samplename(:),samplename2(:));
        glandmorpfeat = glandmorpfeat(I1,:);
        indivglandfeat = indivglandfeat(I1,:);
        samplename = samplename(:,I1);
        classvect = classvect(I1,:);
    end
    %Perform vector quantization on the feature space for each gland and check the
    %histogram of features to do the grading for each core...
    ncores = size(indivglandfeat,1);
    d = size(indivglandfeat{1,1},2); %Number of features for each gland
    totalfeatvect = zeros(0,d);
    for coreidx=1:ncores
        nglands = size(indivglandfeat{coreidx,1},1);
        totalfeatvect(end+1:end+nglands,:)=indivglandfeat{coreidx,1};
    end
    nclusters = 15;
    rmpath(strcat(cd(cd('..')),'\support'));
    opts = statset('Display','iter', 'MaxIter',3000);
    [label,meancenter]=kmeans(totalfeatvect,nclusters,'Options',opts,'Replicates',20,'Start','sample','Display','final');
    
    %Next, compute the histogram of gland index for the sample in each
    %class
    corehist = zeros(ncores,nclusters);
    finalidx=0;
    for coreidx=1:ncores
        nglands = size(indivglandfeat{coreidx,1},1);
        curhist = hist(label(finalidx+1:finalidx+nglands),[1:nclusters]);
        finalidx = finalidx+nglands;
        corehist(coreidx,:)=curhist(:)';
    end
...
%%     %Normalize w.r.t all classes so that the maximum value is 1
    ...
    legend('G2','G3','G4','G5');
    set(gca,'XTickLabel',{'Gland area', 'Peri ratio', 'GEcc','Non-Sold', 'Non-sold max', 'Stromainv','Mean welldef', 'Med welldef', 'Max welldef', 'h1','h2','h3','h4','h5'});
                      drawnow;

    %Do classification on 3 different class based on Gleason grades
    if (useavgfeature)
       [yout]=classify_cancerous_case(glandmorpfeat,cancergrade,samplename);
    else
       [yout]=classify_cancerous_case(corehist,cancergrade,samplename);
    end
    yout;
...
%   P where Ni is the number glands and P is the number of features
      ...
      boundarywidth=30;             %This is the width of the boundary for handling gland's boundary
      ...
      for classidx=1:ngrades %Go through different classes
           if (~ismember(gradelist(classidx),{'NC','SK'})) %Only check the cancer case
               ...
               %for sampleIdx=4:4
                    ...
                    %cur_file_name = 'G:\TMA_cores_and_diagnosis\d3+3\Q11.tif'
                    ...
                    if (strcmp(mode,'gt')==1)
                        ...
                        lblim = imread(strcat(cur_file_name(1:end-4),'_seg_multi_res_3072_strict.tif'));                
                    end
...
%                     
                    ...
                    centroidarr = regionprops(noholeglandmap,'Centroid');
                    ...
                    %fitting ellipse
                    periarr = regionprops(noholeglandmap,'Perimeter'); %3
                    majaxislength = regionprops(noholeglandmap,'MajorAxisLength');
                    minaxislength = regionprops(noholeglandmap,'MinorAxisLength');
                    ...
                    conveximages = regionprops(noholeglandmap,'ConvexImage'); %3
                    boundingboxes  = regionprops(noholeglandmap,'BoundingBox');
                    warning('off');
                    for regionidx=1:nregions
                  ...
                  %  glandmapwidthboundary = imclose(glandmap,strel('disk',maxlumenbandwidthsurroundinggland));
                    ...
                    glandidxlist = regionprops(noholeglandmap,'PixelIdxList');
                    for regionidx=1:nregions
                        curglandidx1 = glandidxlist(regionidx).PixelIdxList;              %Find all 2D coordinates of the current gland
                        [curglandrowidx,curglandcolidx]=ind2sub(size(lblim),curglandidx1);%of the current gland
                        r1 = min(curglandrowidx);
...
function [yout]=classify_cancerous_case(glandmorpfeat,classvect,samplename)
...
%     disp(num2str(cf_matrix));
    ...
    data = glandmorpfeat(idx,:);species = classvect(idx,:);
    [cf_matrix,yout]=classifywithcv(data,species,nround,kval,ctype,samplename(:,idx));
    disp(num2str(cf_matrix));
    
%     disp('Grade 3 vs grade 5 - confusion matrix:')
%     idx = union(find(classvect==3),find(classvect==5));
%     data = glandmorpfeat(idx,:);species = classvect(idx,:);
%     cf_matrix=classifywithcv(data,species,nround,kval,ctype,samplename(:,idx));
%     disp(num2str(cf_matrix));
...
function [cftotalnew,yout]=classifywithcv(data,species,nround,kval,ctype,samplename)
    ...
    nfeat=size(data,2);
    nsamples = size(samplename,2);
    errorcount = zeros(nsamples,1);
    for roundidx=1:nround %Each round start with a different random seeds
         ...
         for i = 1: ntestset;
              ...
              testIdx = find(teIdx==1);
              mismatchidx = find(species(testIdx)~=yout);
              errorcount(testIdx(mismatchidx))=errorcount(testIdx(mismatchidx))+1;
    ...
    erridx=find(errorcount>0);
    errorhist = errorcount(erridx);
    errorsample = samplename(1,erridx);
    trueclass = species(erridx);
    [errorhist,idx] = sort(errorhist,'descend');
    errorsample = errorsample(1,idx);
    trueclass = trueclass(idx);
    nerrorsamples = length(erridx);
    disp('Error samples: (Name - count - true class)');
    for splidx=1:nerrorsamples
        disp([errorsample(splidx) num2str(errorhist(splidx)) num2str(trueclass(splidx))]);
    end
